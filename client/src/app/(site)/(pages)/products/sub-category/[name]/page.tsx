"use client";
import { All_Sub_Categories_BaseUrl } from "@/apiEndpoints";
...
function Page() {
  ...
  const getAllSubCategoresData = async () => {
    try {
      const responseData = await axios.get(
        `${All_Sub_Categories_BaseUrl}&filters[slug][$in]=${name}`
      );
      console.log("responseData in sub ", responseData);

      if (responseData.status === 200) {
        setAllProduct(responseData.data.data[0].products);
      }
    } catch (error) {
  ...
  return (
    <>
      <Breadcrumb
        title={"Explore Sub-Category Products"}
        pages={["products", "/", `sub category-${name}`]}
      />
