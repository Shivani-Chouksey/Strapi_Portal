...
const BlogGrid = ({ params }) => {
  const [bloglist, setBlogsList] = useState([]);
  const getAllBlogs = async () => {
    try {
      const allBlog = await axios.get(
        `http://localhost:1337/api/blogs?populate[author_detail][populate]=profile_image&populate[cover_images]=true&populate[tags]=true&filters[tags][name][$in]=${params.name}`
      );
  ...
  return (
    <>
      <Breadcrumb
        title={`Tag Related Blogs - ${bloglist.length}`}
        pages={["blog grid"]}
      />{" "}
