...
const SingleGridItem = ({ item }) => {
  ...
  const handleQuickViewUpdate = () => {
    // router.push(`shop-with-sidebar/${productId}`);
  ...
  // const validStars = isNaN(rating) ? 0 : Math.max(0, rating);
  const validStars = Math.floor(Number(item.rating) || 0);
  console.log("validStars", validStars);
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          <button
            onClick={() => {
              ...
              // handleQuickViewUpdate(item.documentId);
            }}
            id="newOne"
