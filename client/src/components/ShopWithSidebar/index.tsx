...
import {
  All_Categories_BaseUrl,
  All_Product_BaseUrl,
  All_Sub_Categories_BaseUrl,
} from "@/apiEndpoints";
...
const ShopWithSidebar = () => {
  ...
  const getAllCategores = async () => {
    try {
      const responseData = await axios.get(`${All_Categories_BaseUrl}`);
      ...
      // toast.error(
      //   `${error.response.data.error.message} - Status Code: ${error.response.data.error.status}`
      // );
    }
  };
  const getAllSubCategores = async () => {
    try {
      const responseData = await axios.get(`${All_Sub_Categories_BaseUrl}`);
      if (responseData.status === 200) {
  ...
  const getAllProducts = async () => {
    try {
      const responseData = await axios.get(`${All_Product_BaseUrl}`);
      if (responseData.status === 200) {
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      ...
                      Showing{" "}
                      <span className="text-dark">{allProduct?.length}</span>{" "}
                      Products
                    </p>
                  </div>
