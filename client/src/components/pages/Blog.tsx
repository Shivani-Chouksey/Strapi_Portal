...
const getAllBlogs = async () => {
  try {
    ...
    console.log("allBlog", allBlog.data.data);

    if (allBlog.status === 200) {
      return {
        bloglist: allBlog.data.data,
        paginationInfo: allBlog.data.meta.pagination,
        // seo: allBlog.data.data.seo,
      };
    }
  } catch (error) {
...
const BlogPage = async () => {
  const { bloglist, paginationInfo } = await getAllBlogs();
